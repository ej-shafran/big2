...
// Clay_EndLayout().
CLAY_DLL_EXPORT Clay_RenderCommand* Clay_RenderCommandArray_Get(
...
CLAY_DLL_EXPORT void Clay_SetMaxMeasureTextCacheWordCount(
...
CLAY_DLL_EXPORT void Clay__ConfigureOpenElement(
...
CLAY_DLL_EXPORT Clay_TextElementConfig* Clay__StoreTextElementConfig(
    Clay_TextElementConfig config);
...
Clay__Warning* Clay__WarningArray_Add(Clay__WarningArray* array,
                                      Clay__Warning item);
void* Clay__Array_Allocate_Arena(int32_t capacity,
                                 uint32_t itemSize,
                                 Clay_Arena* arena);
...
Clay__MeasureText(Clay_StringSlice text,
                  Clay_TextElementConfig* config,
                  void* userData);
__attribute__((import_module("clay"),
...
Clay_TextElementConfig* Clay__StoreTextElementConfig(
    Clay_TextElementConfig config) {
  return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
...
Clay_ImageElementConfig* Clay__StoreImageElementConfig(
...
Clay_FloatingElementConfig* Clay__StoreFloatingElementConfig(
...
Clay_CustomElementConfig* Clay__StoreCustomElementConfig(
    Clay_CustomElementConfig config) {
  return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
             ? &Clay_CustomElementConfig_DEFAULT
             : Clay__CustomElementConfigArray_Add(
                   &Clay_GetCurrentContext()->customElementConfigs, config);
}
Clay_ScrollElementConfig* Clay__StoreScrollElementConfig(
    Clay_ScrollElementConfig config) {
  return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
...
Clay_BorderElementConfig* Clay__StoreBorderElementConfig(
...
Clay_SharedElementConfig* Clay__StoreSharedElementConfig(
    Clay_SharedElementConfig config) {
  return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
...
Clay_ElementConfigUnion Clay__FindElementConfigWithType(
...
Clay_ElementId Clay__HashString(Clay_String key,
...
Clay__MeasureTextCacheItem* Clay__MeasureTextCached(
    Clay_String* text,
    Clay_TextElementConfig* config) {
  Clay_Context* context = Clay_GetCurrentContext();
#ifndef CLAY_WASM
...
Clay_LayoutElementHashMapItem* Clay__AddHashMapItem(
    Clay_ElementId elementId,
    Clay_LayoutElement* layoutElement,
    uint32_t idAlias) {
  Clay_Context* context = Clay_GetCurrentContext();
  ...
  while (hashItemIndex != -1) {  // Just replace collision, not a big deal -
    ...
    if (hashItem->elementId.id ==
      ...
      } else {  // Multiple collisions this frame - two elements have the same
                // ID
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
...
Clay_ElementId Clay__GenerateIdForAnonymousElement(
...
void Clay__OpenTextElement(Clay_String text,
  ...
  textElement->minDimensions = CLAY__INIT(Clay_Dimensions){
      ...
      .height = textDimensions.height};  // TODO not sure this is the best way
                                         // to decide min width for text
      ...
      if (sizingAlongAxis) {
        ...
        for (int32_t childOffset = 0;
          ...
          if (!xAxis && Clay__ElementHasConfig(
                            childElement, CLAY__ELEMENT_CONFIG_TYPE_IMAGE)) {
            continue;  // Currently we don't support resizing aspect ratio
                       // images on the Y axis because it would break the ratio
          }
          ...
          switch (elementConfig->type) {
            ...
            case CLAY__ELEMENT_CONFIG_TYPE_IMAGE: {
              ...
              CLAY({.id = CLAY_ID("Clay__DebugViewElementInfoImageBody"),
                ...
                CLAY({.id = CLAY_ID(
                          "Clay__DebugViewElementInfoImageDimensions")}) {
                  CLAY_TEXT(CLAY_STRING("{ width: "), infoTextConfig);
                  CLAY_TEXT(
...
bool Clay_PointerOver(
    Clay_ElementId
        elementId) {  // TODO return priority for separating multiple results
  Clay_Context* context = Clay_GetCurrentContext();
  for (int32_t i = 0; i < context->pointerOverIds.length; ++i) {
    if (Clay__ElementIdArray_Get(&context->pointerOverIds, i)->id ==
        elementId.id) {
